...
class _MenuBaseScreenState extends State<MenuBaseScreen>
            ...
            Padding(
              ...
              child: IconButton(
                ...
                onPressed: () {
                  setState(() {
                    ApiProvider()
                        .delete_Menu_item(list[index].menuId!)
                        .then((value) {
                      setState(() {
                        list.removeAt(index);
                      });
  ...
  Future<BeanSaveMenu?> addBreakfast(int index, String cuisineType,
    ...
    try {
      ...
      // });
      BeanSaveMenu? bean = await ApiProvider().beanSaveMenu(
          isSelectVeg,
          cuisineType,
          nameValidation![index].text,
          priceValidation![index].text,
          await MultipartFile.fromFile(imageValidation!.elementAt(index).path,
              filename: imageValidation.elementAt(index).path));
  ...
  Future<BeanLunchAdd?> addLunch(int index, String cuisineType,
    ...
    try {
      ...
      BeanLunchAdd? bean = await ApiProvider().beanLunchAdd(
          SelectLunchCategory,
          cuisineType,
          nameValidation[index].text,
          priceValidation[index].text,
  ...
  Future<BeanDinnerAdd?> addDinner(int index, String cuisineType,
    ...
    try {
      ...
      BeanDinnerAdd? bean = await ApiProvider().beanDinnerAdd(
  ...
  Future<BeanUpdateMenuStock> getMealScreenItems(
    ...
    BeanUpdateMenuStock? bean = await ApiProvider()
        .updateMenuStock(menuId, (inStock == '0') ? '1' : '0');
